...
function init() {
  ...
  const code = function () {
    ...
    const detectZammad = function () {
      try {
        const zammadObject = App.Config.get("defaults_calendar_subscriptions_tickets");
        return zammadObject.escalation !== undefined && zammadObject.new_open !== undefined && zammadObject.pending !== undefined;
      } catch (error) {
        return false;
      }
    }
    const detectGitlab = function () {
      try {
        return document.getElementsByClassName("detail-page-header-actions js-issuable-actions")[0] !== undefined;
      } catch {
        return false;
      }
    }
    ...
    let harvestTimer = createElement(`<a title="Track Time"  class="harvest-timer" > Track Time</a>`);
    ...
    const zammadAddTimeTracking = function () {
      ...
      function updateTaskName() {
        var taskName = document.getElementsByClassName("ticket-title-update js-objectTitle")[0].textContent;
        var item = { "id": 1337, "name": taskName };
        harvestTimer.setAttribute("data-item", JSON.stringify(item));
      }
      try {
        var bottomBar = document.getElementsByClassName("flex horizontal js-avatars")[0];
        var taskNameDiv = document.getElementsByClassName("ticket-title-update js-objectTitle")[0];
        updateTaskName();
        harvestTimer.href = 'javascript:window.triggerHarvest(); void 0;';
      ...
      window.onhashchange = function () {
        bottomBar = document.getElementsByClassName("flex horizontal js-avatars")[0];
        if (bottomBar && !bottomBar.getElementsByClassName("harvest-timer")[0]) {
          updateTaskName();
          harvestTimer.href = 'javascript:window.triggerHarvest(); void 0;';
    ...
    const jiraAddTimeTracking = function () {
      ...
      if (!harvestTimer) {
        harvestTimer = createElement('<a id="harvest-timer" class="harvest-timer" href="#"></a>');
      ...
      setTimeout(() => {
        ...
        try { // The css classes are generated and may change. To find a sidebar item we are searching for the unique path of its svg icon.
